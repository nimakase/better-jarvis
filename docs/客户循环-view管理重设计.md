...
- ✅ **Bitable 驾驶舱建好 + 接进编排(2026-08-07,真机验通)**:`scripts.bitable_bootstrap` 建 Base「客户循环·贾维斯驾驶舱」+「账户」表(app_token/table_id 进 .env,config 加声明);`prospecting.bitable_client`(按账户名 upsert 只写贾维斯列 / 读回 Ned 手写的 list质量+处置 / 不覆盖);`scripts.bitable_grant` 用 `drive.permission_member` 把 Ned 加成 full_access 协作者。真机验:upsert 幂等、Ned 手写"处置"读得回且 upsert 不覆盖。**接进 `run_view_cycle`**:开头读处置(list质量喂 `core_tier`)、结尾把各账户状态 upsert 进 Bitable(跟 apply 走,dry-run 不写)。
- ✅ **业务记忆两层 + 处置解读器(2026-08-07)**:业务全文 `docs/业务逻辑与工作流程.md`(Ned 确认全对)。**两层注入**——常驻层:`scripts.seed_business_profile` 把 3 条蒸馏事实写进 `core.profile`(每轮注入主对话,不撑大);挂件层:`prospecting.business_context.business_context()` 读全文,**只挂在客户循环 LLM 判断**(已注进 `reply_classify` 的分类 LLM;主 controller prompt 一字不加)。**处置解读器** `prospecting.disposition`(读 Ned 在 Bitable「处置」格的自由文本 → LLM 判 8 意图 protect/switch/snooze/drop/list_quality/done/note/unclear + 整理成一句话,带业务上下文;线程+新loop)。接进 `run_view_cycle`:读懂每格备注 → `dispositions_understood` 进夜报;保护性 Core(Type=Core 无 deal)按 Account Type 分不再误当 prospecting、你批注过就不再提 FYI。安全:意图只驱动贾维斯侧安全动作,绝不因备注写 HubSpot Type/owner。纯层单测绿。
- ✅ **split 改按 Account Type 分**(尊重手动/保护性 Core;Type 空才回退看 deal)。
- **仍待**:处置意图的【深层行为效果】(snooze/drop 真改催发/让decay)+ Bitable「贾维斯理解」回写列(现进夜报);建 Task 落地(reply_path 雏形);priority 退休清理;然后清 store 做干净冷启动。
